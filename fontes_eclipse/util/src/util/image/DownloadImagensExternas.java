...
public class DownloadImagensExternas {
	...
	public DownloadImagensExternas(String tamanho) {
		...
		try {
			for (List<String> listaImagensQuebra : listaImagensGeral) {
				...
				for (String imagem : listaImagensQuebra) {
					DownloadThread downloadThread = new DownloadThread(chave, imagem.replaceAll(".gif", ".jpg"), this.tamanho);
	...
	private boolean imagemExiste(String imagem) {
		try {
			...
			HttpURLConnection con = (HttpURLConnection) new URL(
					"http://s3.sa-east-1.amazonaws.com/imagens.catalogobeirario.com.br/"+((this.tamanho.equals("A")?"alta":"angulos_produtos"))+"/" + imagem).openConnection();
	...
	public class DownloadThread implements Runnable {
		...
		DownloadThread(String chave, String imagem, String tamanho) {
			this.chave = chave;
			this.imagem = imagem;
			this.tamanho = tamanho;
		}
		...
		public void run() {
			try {
				FileUtils.copyURLToFile(
						new URL("http://s3.sa-east-1.amazonaws.com/imagens.catalogobeirario.com.br/"+((this.tamanho.equals("A")?"alta":"angulos_produtos"))+"/"+ this.imagem),
