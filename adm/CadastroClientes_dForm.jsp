          ...
          <td class="mainTd">
            ...
            <table id="TRbl_report_Jw_clientes_cad_dados" class="reportTable" style=width:99%; border=1; >
              ...
              try {
                ...
                if(!(CadastroClientes_dActionForm.getCli_cdgo()+"").equals("null")){
                  ...
                  query1.append("        ,(SELECT replace(rowtocol(' ");
                  query1.append("SELECT mp.descricao_marca ");
                  ...
                  query1.append("   AND situacao = ''A'' ");
                  query1.append("   and not exists(select 1 from cliente_representante_marca crm where crm.status = ''A'' and cli_cdgo = '''||cr.codigo_cliente||''' and esc_seqn = '''||cr.codigo_sequencial||''' and rep_cdgo = '''||cr.codigo_representante||''') ");
                  query1.append("UNION ");
                  query1.append("SELECT pck_marcas_produtos.get_nome(crm.codigo_marca) descricao_marca ");
                  query1.append("  FROM cliente_representante_marca crm ");
                  query1.append(" WHERE crm.status = ''A'' ");
                  query1.append("   AND crm.cli_cdgo = '''||cr.codigo_cliente||''' ");
                  query1.append("   and crm.esc_seqn = '''||cr.codigo_sequencial||''' ");
                  query1.append("   and crm.rep_cdgo = '''||cr.codigo_representante||''' ");
                  query1.append("   '),',',', ') FROM dual) marcas  ");
                  ...
                  query1.append("    AND  cr.situacao <> 'R'");
                ...
                if (res_Jw_clientes_cad_dados!=null && res_Jw_clientes_cad_dados.size()>0) {
                  ...
                  query2.append(" SELECT CASE ");
                  query2.append("         WHEN (SELECT NVL(telefone1, telefone2) telefone ");
                  query2.append("                 FROM rep_inf_adicionais ");
                  query2.append("                WHERE rep_cdgo = '"+codigo_representante+"') IS NULL THEN ");
                  query2.append("          (SELECT LPAD(ffs_ddd, 3, '0') || '-' || ffs_nmro fone ");
                  query2.append("             FROM fax_fon ");
                  query2.append("            WHERE rep_cdgo = '"+codigo_representante+"' ");
                  query2.append("              AND rep_cdgo > 0 ");
                  query2.append("              AND (ffs_tipo = 7 OR ffs_tipo = 1)) ");
                  query2.append("         ELSE ");
                  query2.append("          (SELECT NVL(telefone1, telefone2) telefone ");
                  query2.append("             FROM rep_inf_adicionais ");
                  query2.append("            WHERE rep_cdgo = '"+codigo_representante+"') ");
                  query2.append("       END fone ");
                  query2.append("  FROM dual ");
                  ...
                  while (i_bl_report_Jw_clientes_cad_dados<res_Jw_clientes_cad_dados.size()){
                    ...
                    query3.append(" SELECT CASE ");
                    query3.append("         WHEN (SELECT NVL(telefone1, telefone2) telefone ");
                    query3.append("                 FROM rep_inf_adicionais ");
                    query3.append("                WHERE rep_cdgo = '"+codigo_representante+"') IS NULL THEN ");
                    query3.append("          (SELECT LPAD(ffs_ddd, 3, '0') || '-' || ffs_nmro fone ");
                    query3.append("             FROM fax_fon ");
                    query3.append("            WHERE rep_cdgo = '"+codigo_representante+"' ");
                    query3.append("              AND rep_cdgo > 0 ");
                    query3.append("              AND (ffs_tipo = 7 OR ffs_tipo = 1)) ");
                    query3.append("         ELSE ");
                    query3.append("          (SELECT NVL(telefone1, telefone2) telefone ");
                    query3.append("             FROM rep_inf_adicionais ");
                    query3.append("            WHERE rep_cdgo = '"+codigo_representante+"') ");
                    query3.append("       END fone ");
                    query3.append("  FROM dual ");
            ...
            <html:form action="adm/CadastroClientes_dForm.do" method="post" styleClass="baseForm">
              ...
              <table class="itemTable"  style="width:99%;">
                ...
                portalbr.adm.CadastroClientes_dActionForm cadastroClientes = (portalbr.adm.CadastroClientes_dActionForm)session.getAttribute("CadastroClientes_dActionForm");
                ...
                <tr class="itemTr">
                  ...
                  <td class="formField">
                    ...
                    <%
                    if(cadastroClientes.getCodigo_motivo() != "") {
                      %>
                      <span class="spamFormLabel">
                        Motivo Bloqueio
                      </span>
                      <html:text property="codigo_motivo" styleId="codigo_motivo" styleClass="disabled" tabindex="-1" readonly="true" size="3"/>
                      <span class="spamFormLabel">
                      </span>
                      <html:text property="descricao_motivo" styleId="descricao_motivo" styleClass="disabled" tabindex="-1" readonly="true" size="35"/>
                    </td>
                    <%
                  } else {
                    %>

                    <html:hidden property="codigo_motivo" />
                    <html:hidden property="descricao_motivo" />
                        ...
                        <table id="TRbl_report_Jw_clientes_enderecos" class="reportTable" style=width:90%; border=1;  >
                          ...
                          int counterbl_report_Jw_clientes_enderecos=0;
                          {
                            ...
                            if (res_Jw_clientes_enderecos!=null && res_Jw_clientes_enderecos.size()>0) {
                              ...
                              %>
                              <tr class="reportHeader3" colspan="5"; >
