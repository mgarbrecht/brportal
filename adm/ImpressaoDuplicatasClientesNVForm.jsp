          ...
          <td class="mainTd">
            ...
            <table id="TRbl_report_Jw_consulta_titulos" class="reportTable" style=width:99%;  border=1  frame=box>
              ...
              int counterbl_report_Jw_consulta_titulos=0;
              {
                ...
                if (res_Jw_consulta_titulos!=null && res_Jw_consulta_titulos.size()>0) {
                  ...
                  java.lang.String esc_situacao           = null;
                  java.lang.String descricao_esc_situacao = null;
                  ...
                  esc_situacao           = t_jw_consulta_titulos.getEsc_situacao()==null?"":t_jw_consulta_titulos.getEsc_situacao();
                  descricao_esc_situacao = t_jw_consulta_titulos.getDescricao_esc_situacao()==null?"":t_jw_consulta_titulos.getDescricao_esc_situacao();
                  %>
                    ...
                    <tr class=<%= style %> id='TRbl_report_Jw_consulta_titulos<%=counterbl_report_Jw_consulta_titulos%>' >
                      <td class="reportColumn" style="text-align:left;" title="Títulos do Cliente: <%= razao %>">
                        ...
                        if(codigo_sucessor.equals("null")||codigo_sucessor.equals("")) {
                          %>
                          <a href="../adm/ImpressaoDuplicatasNV_rForm.do?select_action=&esc_seqn=<%= esc_seqn %>&cli_cdgo=<%= cli_cdgo %>&dt_venc=<%= ImpressaoDuplicatasClientesNVActionForm.getDt_venc() %>&dt_venc_final=<%= ImpressaoDuplicatasClientesNVActionForm.getDt_venc_final() %>&situacao=<%= ImpressaoDuplicatasClientesNVActionForm.getSituacao() %>&codigo=<%= cliente %>&nome=<%= razao %>&bairro=<%= br %>&cidade_uf=<%= cid_est %>&emp_empresa=<%= ImpressaoDuplicatasClientesNVActionForm.getEmp_empresa() %>" target="_self" STYLE="cursor:hand" onclick="bloqueia();">
                            <%= cliente %>
                          </a>
                          <%
                        } else {
                          ...
                          <a href="../adm/ImpressaoDuplicatasNV_rForm.do?select_action=&esc_seqn=<%= sequencial_sucessor %>&cli_cdgo=<%= codigo_sucessor %>&dt_venc=<%= ImpressaoDuplicatasClientesNVActionForm.getDt_venc() %>&dt_venc_final=<%= ImpressaoDuplicatasClientesNVActionForm.getDt_venc_final() %>&situacao=<%= ImpressaoDuplicatasClientesNVActionForm.getSituacao() %>&codigo=<%= codigo_sucessor %>&nome=<%= razao_social_sucessor %>&bairro=<%= br_sucessor %>&cidade_uf=<%= cid_uf_sucessor  %>&emp_empresa=<%= ImpressaoDuplicatasClientesNVActionForm.getEmp_empresa() %>" target="_self" STYLE="cursor:hand" onclick="bloqueia();">
                            <%= codigo_sucessor %>
                            -
                            <%= sequencial_sucessor %>
                          </a>
            ...
            <html:form action="adm/ImpressaoDuplicatasClientesNVForm.do" method="post" styleClass="baseForm">
              <table class="itemTable"  style=" width:;">
                ...
                <html:hidden property="dt_venc_final"/>
