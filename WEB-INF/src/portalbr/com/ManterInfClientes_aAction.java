package portalbr.com;

import java.util.*;
import javax.servlet.*;
import javax.servlet.http.*;
import org.apache.struts.action.*;
import com.egen.util.jdbc.*;
import com.egen.util.text.*;
import portalbr.dbobj.table.*;
import portalbr.dbobj.view.*;
import portalbr.dbobj.procedure.*;

/**
 * Creation Date 14/12/2011 10:52:54
 * Last Modify Date 14/12/2011 11:14:51
 */

public class ManterInfClientes_aAction extends com.egen.util.struts.AbstractAction {

  public ActionForward perform_update_action(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
    HttpSession session = request.getSession(true);
    ActionForward actionForward = null;
    com.egen.util.jdbc.JdbcUtil j = null;
    try {
      j = new com.egen.util.jdbc.JdbcUtil();
      ManterInfClientes_aActionForm f = (ManterInfClientes_aActionForm) form;
      try {
        update_action(j, mapping, form, request, response);
      }
      finally {
      }
      actionForward = new ActionForward("/com/ManterInfClientesForm.do?select_session_action=",true);
    } catch (Exception e) {
      ActionErrors errors = new ActionErrors();
      errors.add("ActionErrors.GLOBAL_ERROR_bl_form_Cli_inf_adicionais", new ActionMessage("error.action.exception",com.egen.util.system.Error.getMessage(e)));
      request.setAttribute(org.apache.struts.Globals.ERROR_KEY, errors);
      session.setAttribute("exception", com.egen.util.system.Error.getDescription(e));
      actionForward = mapping.findForward("same");
    } finally {
      if (j != null) {
        j.close();
      }
    }
    return actionForward;
  }
  private void update_action(JdbcUtil j, ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
    HttpSession session = request.getSession(true);
    ManterInfClientes_aActionForm f = (ManterInfClientes_aActionForm) form;
    Cli_inf_adicionais table = new Cli_inf_adicionais();
    Vector sets = new Vector(10, 2);
    table.setCli_cdgo(com.egen.util.text.FormatNumber.parseIntegerWrapper(f.getCli_cdgo()));
    if (f.getCli_cdgo() != null) {
      sets.addElement("cli_cdgo");
    }
    table.setEsc_seqn(com.egen.util.text.FormatNumber.parseIntegerWrapper(f.getEsc_seqn()));
    if (f.getEsc_seqn() != null) {
      sets.addElement("esc_seqn");
    }
    table.setSomente_etiqueta(f.getSomente_etiqueta());
    if (f.getSomente_etiqueta() != null) {
      sets.addElement("somente_etiqueta");
    }
    if (sets != null && sets.size() > 0) {
      String[] set = new String[sets.size()];
      for (int i = 0; i < sets.size(); i++) {
        set[i] = (String) sets.elementAt(i);
      }
      int updated = j.update(table, set);
      if (updated == 0) {
        ActionErrors errors = new ActionErrors();
        errors.add("ActionErrors.GLOBAL_ERROR_bl_form_Cli_inf_adicionais", new ActionMessage("warn.norowsupdated",": " + updated));
        request.setAttribute(org.apache.struts.Globals.ERROR_KEY, errors);
      } else {
        Vector res_Cli_inf_adicionais = (Vector) session.getAttribute("res_Cli_inf_adicionais");
        if (res_Cli_inf_adicionais != null) {
          res_Cli_inf_adicionais.setElementAt(table, 0);
          Cli_inf_adicionais r = (Cli_inf_adicionais) res_Cli_inf_adicionais.elementAt(0);
          f.setCli_cdgo((r.getCli_cdgo() != null) ? r.getCli_cdgo() + "":"");
          f.setEsc_seqn((r.getEsc_seqn() != null) ? r.getEsc_seqn() + "":"");
          f.setSomente_etiqueta((r.getSomente_etiqueta() != null) ? r.getSomente_etiqueta() + "":"");
        }
      }
    }
    session.setAttribute(mapping.getName(), form);
  }

  public ActionForward perform_delete_action(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
    HttpSession session = request.getSession(true);
    ActionForward actionForward = null;
    com.egen.util.jdbc.JdbcUtil j = null;
    try {
      j = new com.egen.util.jdbc.JdbcUtil();
      ManterInfClientes_aActionForm f = (ManterInfClientes_aActionForm) form;
      try {
        delete_action(j, mapping, form, request, response);
      }
      finally {
      }
      actionForward = new ActionForward("/com/ManterInfClientesForm.do?select_session_action=",true);
    } catch (Exception e) {
      ActionErrors errors = new ActionErrors();
      errors.add("ActionErrors.GLOBAL_ERROR_bl_form_Cli_inf_adicionais", new ActionMessage("error.action.exception",com.egen.util.system.Error.getMessage(e)));
      request.setAttribute(org.apache.struts.Globals.ERROR_KEY, errors);
      session.setAttribute("exception", com.egen.util.system.Error.getDescription(e));
      actionForward = mapping.findForward("same");
    } finally {
      if (j != null) {
        j.close();
      }
    }
    return actionForward;
  }
  private void delete_action(JdbcUtil j, ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
    HttpSession session = request.getSession(true);
    ManterInfClientes_aActionForm f = (ManterInfClientes_aActionForm) form;
    Cli_inf_adicionais table = new Cli_inf_adicionais();
    table.setCli_cdgo(com.egen.util.text.FormatNumber.parseIntegerWrapper(f.getCli_cdgo()));
    table.setEsc_seqn(com.egen.util.text.FormatNumber.parseIntegerWrapper(f.getEsc_seqn()));
    table.setSomente_etiqueta(f.getSomente_etiqueta());
    j.delete(table);
    Vector res_Cli_inf_adicionais = (Vector) session.getAttribute("res_Cli_inf_adicionais");
    if (res_Cli_inf_adicionais != null && res_Cli_inf_adicionais.size() > 0) {
      res_Cli_inf_adicionais.removeElementAt(0);
      if (res_Cli_inf_adicionais.size() > 0) {
        Cli_inf_adicionais r = (Cli_inf_adicionais) res_Cli_inf_adicionais.elementAt(0);
        f.setCli_cdgo((r.getCli_cdgo() != null) ? r.getCli_cdgo() + "":"");
        f.setEsc_seqn((r.getEsc_seqn() != null) ? r.getEsc_seqn() + "":"");
        f.setSomente_etiqueta((r.getSomente_etiqueta() != null) ? r.getSomente_etiqueta() + "":"");
      } else {
        f.reset(mapping, request);
      }
    } else {
      f.reset(mapping, request);
    }
    session.setAttribute(mapping.getName(), form);
  }

  public ActionForward perform_back_action(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
    HttpSession session = request.getSession(true);
    ActionForward actionForward = null;
    com.egen.util.jdbc.JdbcUtil j = null;
    try {
      j = new com.egen.util.jdbc.JdbcUtil();
      ManterInfClientes_aActionForm f = (ManterInfClientes_aActionForm) form;


      actionForward = new ActionForward("/com/ManterInfClientesForm.do?select_session_action=",true);
    } catch (Exception e) {
      ActionErrors errors = new ActionErrors();
      errors.add("ActionErrors.GLOBAL_ERROR_bl_form_Cli_inf_adicionais", new ActionMessage("error.action.exception",com.egen.util.system.Error.getMessage(e)));
      request.setAttribute(org.apache.struts.Globals.ERROR_KEY, errors);
      session.setAttribute("exception", com.egen.util.system.Error.getDescription(e));
      actionForward = mapping.findForward("same");
    } finally {
      if (j != null) {
        j.close();
      }
    }
    return actionForward;
  }
  public ActionForward perform_select_action(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
    HttpSession session = request.getSession(true);
    ActionForward actionForward = null;
    com.egen.util.jdbc.JdbcUtil j = null;
    try {
      j = new com.egen.util.jdbc.JdbcUtil();
      ManterInfClientes_aActionForm f = (ManterInfClientes_aActionForm) form;
      try {
        select_action(j, mapping, form, request, response);
      }
      finally {
      }
      actionForward = mapping.findForward("same");
    } catch (Exception e) {
      ActionErrors errors = new ActionErrors();
      errors.add("ActionErrors.GLOBAL_ERROR_bl_form_Cli_inf_adicionais", new ActionMessage("error.action.exception",com.egen.util.system.Error.getMessage(e)));
      request.setAttribute(org.apache.struts.Globals.ERROR_KEY, errors);
      session.setAttribute("exception", com.egen.util.system.Error.getDescription(e));
      actionForward = mapping.findForward("same");
    } finally {
      if (j != null) {
        j.close();
      }
    }
    return actionForward;
  }
  private void select_action(JdbcUtil j, ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
    HttpSession session = request.getSession(true);
    ManterInfClientes_aActionForm f = (ManterInfClientes_aActionForm) form;
    Cli_inf_adicionais table = new Cli_inf_adicionais();
    String[][] select = null;
    String[] order = null;
    String[] groupby = null;
    String[] having = null;
    Object[][] where = { {"cli_cdgo","=", com.egen.util.text.FormatNumber.toInteger(f.getCli_cdgo())}, {"esc_seqn","=", com.egen.util.text.FormatNumber.toInteger(f.getEsc_seqn())}, {"somente_etiqueta","like", f.getSomente_etiqueta()} };
    Vector res_Cli_inf_adicionais = j.select(table, select, where, groupby, having, order);
    session.setAttribute("res_Cli_inf_adicionais",res_Cli_inf_adicionais);
    if (res_Cli_inf_adicionais != null && res_Cli_inf_adicionais.size() > 0) {
      Cli_inf_adicionais r = (Cli_inf_adicionais) res_Cli_inf_adicionais.elementAt(0);
      f.setCli_cdgo((r.getCli_cdgo() != null) ? r.getCli_cdgo() + "":"");
      f.setEsc_seqn((r.getEsc_seqn() != null) ? r.getEsc_seqn() + "":"");
      f.setSomente_etiqueta((r.getSomente_etiqueta() != null) ? r.getSomente_etiqueta() + "":"");
    } else {
      ActionErrors errors = (ActionErrors) request.getAttribute(org.apache.struts.Globals.ERROR_KEY);
      if (errors == null) {
        errors = new ActionErrors();
      }
      errors.add("ActionErrors.GLOBAL_ERROR_bl_form_Cli_inf_adicionais", new ActionMessage("warn.norowsselected",""));
      request.setAttribute(org.apache.struts.Globals.ERROR_KEY, errors);
    }
    session.setAttribute(mapping.getName(), form);
  }

}
