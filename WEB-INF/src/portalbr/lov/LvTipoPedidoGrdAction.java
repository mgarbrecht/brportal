...
public class LvTipoPedidoGrdAction extends com.egen.util.struts.AbstractAction {
  ...
  public ActionForward perform_select_action(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
    ...
    try {
      ...
      LvTipoPedidoGrdActionForm[]_ge_grid_array = (LvTipoPedidoGrdActionForm[])_ge_grid.get("bl_form_Jw_tipo_pedido_lines");
      LvTipoPedidoGrdActionForm f = _ge_grid_array[0];
      try {
      ...
      errors.add("ActionErrors.GLOBAL_ERROR_bl_form_Jw_tipo_pedido", new ActionMessage("error.action.exception",com.egen.util.system.Error.getMessage(e)));
  ...
  private void select_action(JdbcUtil j, ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
    ...
    LvTipoPedidoGrdActionForm f = _ge_grid_array[0];
    if (f != null) {
      Jw_tipo_pedido table = new Jw_tipo_pedido();
      String[][] select = null;
      String[] order = null;
      String[] groupby = null;
      String[] having = null;
      Object[][] where = { {"tipo_pedido","like", f.getTipo_pedido()}, {"descricao_tipo_pedido","like", f.getDescricao_tipo_pedido()} };
      Vector res_Jw_tipo_pedido = j.select(table, select, where, groupby, having, order);
      session.setAttribute("res_Jw_tipo_pedido",res_Jw_tipo_pedido);
      if (res_Jw_tipo_pedido != null && res_Jw_tipo_pedido.size() > 0) {
        LvTipoPedidoGrdActionForm[]_ge_grid_array_flex = new LvTipoPedidoGrdActionForm[res_Jw_tipo_pedido.size()];
        org.apache.struts.config.ConfigEGenHelper.setFormPropertySize(mapping, "bl_form_Jw_tipo_pedido_lines", res_Jw_tipo_pedido.size());
        int _ge_pos = 0;
        for (int _ge_grid_array_index = 0; _ge_grid_array_index < _ge_grid_array_flex.length; _ge_grid_array_index++, _ge_pos++) {
          if (_ge_grid_array_index < _ge_grid_array.length) {
            ...
            f = new LvTipoPedidoGrdActionForm();
          ...
          if (res_Jw_tipo_pedido.size() > _ge_pos) {
            Jw_tipo_pedido r = (Jw_tipo_pedido) res_Jw_tipo_pedido.elementAt(_ge_pos);
            f.setTipo_pedido((r.getTipo_pedido() != null) ? r.getTipo_pedido() + "":"");
            f.setDescricao_tipo_pedido((r.getDescricao_tipo_pedido() != null) ? r.getDescricao_tipo_pedido() + "":"");
          } else {
            f.reset(mapping, request);
          }
          _ge_grid_array_flex[_ge_grid_array_index] = f;
        }
        _ge_grid.set("bl_form_Jw_tipo_pedido_lines", _ge_grid_array_flex);
      } else {
        ActionErrors errors = (ActionErrors) request.getAttribute(org.apache.struts.Globals.ERROR_KEY);
        if (errors == null) {
          errors = new ActionErrors();
        }
        errors.add("ActionErrors.GLOBAL_ERROR_bl_form_Jw_tipo_pedido", new ActionMessage("warn.norowsselected",""));
        request.setAttribute(org.apache.struts.Globals.ERROR_KEY, errors);
      }
  ...
  public ActionForward perform_resetfull_action(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
    ...
    try {
      ...
      LvTipoPedidoGrdActionForm[]_ge_grid_array = (LvTipoPedidoGrdActionForm[])_ge_grid.get("bl_form_Jw_tipo_pedido_lines");
      LvTipoPedidoGrdActionForm f = _ge_grid_array[0];
      session.removeAttribute("res_Jw_tipo_pedido");
      com.egen.util.struts.AbstractActionHelper.resetForm("bl_form_Jw_tipo_pedido_lines", mapping, form, request);
      session.setAttribute(mapping.getName(), form);
      actionForward = mapping.findForward("same");
    } catch (Exception e) {
      ActionErrors errors = new ActionErrors();
  ...
  private void update_action(JdbcUtil j, ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
    ...
    for (int _ge_grid_array_index = 0; _ge_grid_array_index < _ge_grid_array.length; _ge_grid_array_index++) {
      ...
      if (f.getGridCheckField()) {
        if (tipoPedidos.equals("")) {
          vetor.addElement(f.getTipo_pedido() + "-"+f.getDescricao_tipo_pedido());
          tipoPedidos = f.getTipo_pedido();
        } else {
          vetor.addElement(f.getTipo_pedido() + "-"+f.getDescricao_tipo_pedido());
          tipoPedidos += ","+f.getTipo_pedido();
        }
    ...
    session.setAttribute("tipos_pedidos_selecionados_vetor", vetor);
    session.setAttribute("tipos_pedidos_selecionados_codigos", tipoPedidos);
    request.setAttribute("tipos_pedidos_selecionados", tipoPedidos);
  }
  ...
  private void tg_marcar_todos_action(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
    ...
    try {
      ...
      if (_ge_grid_array != null) {
        LvTipoPedidoGrdActionForm f = null;
        for (int _ge_grid_array_index = 0; _ge_grid_array_index < _ge_grid_array.length; _ge_grid_array_index++) {
          f = _ge_grid_array[_ge_grid_array_index];
