...
public class TitulosAction extends com.egen.util.struts.AbstractAction {

  public ActionForward perform_select_action(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
    //------------------------------------------------------------------------------------------
    // Autor             : Mateus Garbrecht (TechnoCorp)
    // Data de Criação   : 20/03/2006
    // Objetivo          : Executa uma consulta na view portalbr.dbobj.view.Jw_consulta_titulos
    //------------------------------------------------------------------------------------------
    // Data de Alteração :
    // Descrição         :
    //------------------------------------------------------------------------------------------
    ...
    TitulosActionForm f = (TitulosActionForm) form;
    int quantidadeTitulosConsulta = 0;
    try {

      if ((!(f.getCli_rzao() != "")) && (!(f.getCli_cdgo() != "")) && (!(f.getTit_codigo() != "")) && (!(f.getCgc() != ""))) {
        throw new Exception("Um dos parâmetros obrigatórios deve ser informado!");
      }

      if (f.getTit_codigo() != "") {
        String titulos = f.getTit_codigo().replaceAll("(;|,|:)", " ").trim().toUpperCase();
        titulos = titulos.replaceAll("\\s+", " ");
        titulos = titulos.replaceAll(" ", "\n");
        titulos = titulos.replaceAll("\\r\\n|\\r|\\n", ",");
        if ((titulos.charAt(titulos.length() - 1) + "").equalsIgnoreCase(",")) {
          titulos = titulos.substring(0, titulos.lastIndexOf(","));
        }
        String[] qtd = titulos.split(",");
        quantidadeTitulosConsulta = qtd.length;
        if (quantidadeTitulosConsulta > 1) {
          session.setAttribute("consulta_titulos_multiplos_registros", titulos.toUpperCase());
        }
      }

      if (quantidadeTitulosConsulta == 0) {
        ...
        if (f.getCli_rzao() != "") {
          if ((f.getReunir() + "").equals("1")) {
            String tipo = "";
            String cliente_mercado_interno = "";
            ...
            if (!(u.getEntidadeNegocio() == null)) {
              ...
              if (tipo.equals("CLIENTE MERCADO INTERNO")) {
                cliente_mercado_interno = ent.getChave() + "";
              }
            }


            actionForward = new ActionForward("/adm/Titulos_rForm.do?select_action=&cli_cdgo="+cliente_mercado_interno + "&dt_venc="+f.getDt_vencimento() + "&situacao="+f.getSituacao() + "&codigo="+cliente_mercado_interno + "&nome="+f.getNome() + "&bairro="+f.getBairro() + "&cidade_uf="+f.getCidade_uf() + "&emp_empresa="+f.getEmp_empresa(), true);

          } else {

            actionForward = new ActionForward("/adm/TitulosClientesForm.do?&situacao="+f.getSituacao() + "&dt_venc="+f.getDt_vencimento() + "&emp_empresa="+f.getEmp_empresa(), true);
        ...
        if (!(f.getCli_rzao() != "")) {
          if ((f.getReunir() + "").equals("1")) {
            actionForward = new ActionForward("/adm/Titulos_rForm.do?select_action=&cli_cdgo="+f.getCli_cdgo() + "&dt_venc="+f.getDt_vencimento() + "&situacao="+f.getSituacao() + "&codigo="+f.getCli_cdgo() + "&nome="+f.getNome() + "&bairro="+f.getBairro() + "&cidade_uf="+f.getCidade_uf() + "&emp_empresa="+f.getEmp_empresa(), true);

          } else {
            if (f.getEsc_seqn() != "") {
              actionForward = new ActionForward("/adm/Titulos_rForm.do?select_action=&esc_seqn="+f.getEsc_seqn() + "&cli_cdgo="+f.getCli_cdgo() + "&dt_venc="+f.getDt_vencimento() + "&situacao="+f.getSituacao() + "&codigo="+f.getCodigo() + "&nome="+f.getNome() + "&bairro="+f.getBairro() + "&cidade_uf="+f.getCidade_uf() + "&emp_empresa="+f.getEmp_empresa(), true);
        ...
        if (quantidadeTitulosConsulta == 1) {
          actionForward = new ActionForward("/adm/TitulosForm.do?select2_action=&tit_codigo=" + f.getTit_codigo(), true);
        } else {
          actionForward = new ActionForward("/adm/Titulos_rForm.do?select_action=", true);
        }
      }
  ...
  private void select_action(JdbcUtil j, ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
    ...
    if (f.getCgc() != null && f.getCgc() != "" && f.getCgc() != "null") {
      ...
      String cliente = qr.retorna(j, "select cli_cdgo FROM esc WHERE esc_situacao = 'A' and esc_cgc = " + f.getCgc());
      String sequencial = qr.retorna(j, "select esc_seqn FROM esc WHERE esc_situacao = 'A' and esc_cgc = " + f.getCgc());
    ...
    if (f.getRepresentante() != "") {
      ...
      query.append(" SELECT a.cli_cdgo||'-'||b.esc_seqn cliente ");
      ...
      //   query.append("    AND nvl(b.esc_situacao,NULL) = 'A' ");
  ...
  public ActionForward perform_select2_action(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
    ...
    try {
      ...
      if (clienteFiltro != null) {
        query.append(" AND t.cli_codigo = " + clienteFiltro);
      }
