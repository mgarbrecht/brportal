 ...
 * Last Modify Date 13/02/2020 15:34:59
 */

public class Clientes_regionalAction extends com.egen.util.struts.AbstractAction {
  ...
  private void select_action(JdbcUtil j, ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
    ...
    Clientes_regionalActionForm f = (Clientes_regionalActionForm) form;
    portalbr.dbobj.view.Clientes_regional table = new portalbr.dbobj.view.Clientes_regional();
    ...
    query.append(" SELECT /*+ all_rows */ regional ");
    ...
    query.append(" FROM( ");
    query.append(" SELECT lpad(res.codigo_regional, 3, 0) || '-' || rv.nome_regional regional ");
    ...
    query.append("    AND res.codigo_marca         =  nvl(").append(session.getAttribute("codigo_marca")).append(",res.codigo_marca) ");
    query.append("    AND rm.marca_produto         =  res.codigo_marca )  ");
    query.append("  GROUP BY regional ");
    query.append("         , codigo_regional ");
    if (session.getAttribute("ca_abrir_meses") == "S") {
    ...
    Vector res_Clientes_regional = j.select(table, query.toString(), null);


    if (res_Clientes_regional != null && res_Clientes_regional.size() > 0) {
      for (int x = 0; x < res_Clientes_regional.size(); x++) {
        portalbr.dbobj.view.Clientes_regional r = (portalbr.dbobj.view.Clientes_regional) res_Clientes_regional.elementAt(x);
        //-----grupos ativos-------
        StringBuilder sbga = new StringBuilder();
        sbga.append(" SELECT /*+ all_rows */ count(distinct grec.gre_grupo) qtd_grupos ");
        sbga.append("   FROM cliente_representante clre ");
        sbga.append("      , grupo_economico       grec ");
        sbga.append("      , representante_marca   rema ");
        sbga.append("      , rep                   repr ");
        sbga.append("      , regionais_vendas      reve ");
        sbga.append("      , marcas_produtos       mapr ");
        sbga.append("  WHERE clre.codigo_representante = repr.rep_cdgo ");
        sbga.append("    AND grec.cli_cdgo             = clre.codigo_cliente ");
        sbga.append("    AND rema.codigo_representante = repr.rep_cdgo ");
        sbga.append("    AND reve.codigo_regional      = repr.codigo_regional ");
        sbga.append("    AND mapr.codigo_marca         = rema.marca_produto ");
        sbga.append("    AND repr.rep_situ             = nvl(").append(session.getAttribute("situacao_representante")).append(",repr.rep_situ) ");
        sbga.append("    AND clre.situacao             = nvl(").append(session.getAttribute("situacao_cliente_representante")).append(",clre.situacao) ");
        sbga.append("    AND rema.situacao             = nvl(").append(session.getAttribute("situacao_representante_marca")).append(",rema.situacao) ");
        sbga.append("    AND mapr.codigo_marca         = nvl(").append(session.getAttribute("codigo_marca")).append(",mapr.codigo_marca) ");
        sbga.append("    AND reve.codigo_regional      = ").append(r.getCodigo_regional());


        if (session.getAttribute("ca_consulta_grupos") == "S") {
          Vector res_Grupos_ativos = j.select(table_grupos_ativos, sbga.toString(), null);
        ...
        if (session.getAttribute("ca_abrir_meses") == "S") {
          grupos_atendidos.append("  WHERE res.data_dig >= to_date('"+r.getMes() + "', 'yyyy-mm-dd hh24:mi:ss') ");
          grupos_atendidos.append("  AND res.data_dig < last_day(to_date('"+r.getMes() + "', 'yyyy-mm-dd hh24:mi:ss')) + 1 ");
        } else {
        ...
        sbca.append(" SELECT /*+ all_rows */ count(distinct cli.cli_cdgo) ativos_qtd ");
        sbca.append("   FROM rep ");
        sbca.append("      , cliente_representante clr ");
        sbca.append("      , cli ");
        sbca.append("      , representante_marca rm ");
        sbca.append("  WHERE clr.codigo_representante =  rep.rep_cdgo ");
        sbca.append("    AND cli.cli_cdgo             =  clr.codigo_cliente ");
        sbca.append("    AND rm.codigo_representante  =  rep.rep_cdgo ");
        sbca.append("    AND cli.cli_situ             =  nvl('A', cli.cli_situ) ");
        sbca.append("    AND cli.seg_cdgo             NOT IN (9,153) ");
        sbca.append("    AND rep.rep_situ             =  nvl(").append(session.getAttribute("situacao_representante")).append(",rep.rep_situ) ");
        sbca.append("    AND clr.situacao             =  nvl(").append(session.getAttribute("situacao_cliente_representante")).append(",clr.situacao) ");
        sbca.append("    AND rm.situacao              =  nvl(").append(session.getAttribute("situacao_representante_marca")).append(",rm.situacao) ");
        sbca.append("    AND rep.codigo_regional      =  nvl(").append(r.getCodigo_regional()).append(",rep.codigo_regional) ");
        sbca.append("    AND rm.marca_produto         =  nvl(").append(session.getAttribute("codigo_marca")).append(",rm.marca_produto) ");

        if (session.getAttribute("ca_consulta_clientes") == "S") {
          Vector res_Clientes_ativos = j.select(table_clientes_ativos, sbca.toString(), null);
        ...
        /*
          StringBuilder sbcatr = new StringBuilder();
          sbcatr.append(" SELECT COUNT(distinct cli_cdgo) atendidos_qtd ");
          sbcatr.append("   FROM (  ");
          sbcatr.append(" SELECT cli.cli_cdgo ");
          sbcatr.append("   FROM resumo_pedidos res ");
          sbcatr.append("      , cli ");
          sbcatr.append("      , rep ");
          sbcatr.append("      , cliente_representante clr ");
          sbcatr.append("      , representante_marca rm ");
          if (session.getAttribute("ca_abrir_meses") == "S") {
              sbcatr.append("  WHERE res.data_dig >= to_date('"+r.getMes()+"', 'yyyy-mm-dd hh24:mi:ss') ");
              sbcatr.append("  AND res.data_dig  < last_day(to_date('"+r.getMes()+"', 'yyyy-mm-dd hh24:mi:ss')) + 1 ");
          }
          else{
              sbcatr.append("  WHERE res.data_dig             >= to_date('").append(session.getAttribute("dt_inicial")).append("','dd/MM/yyyy') ");
              sbcatr.append("    AND res.data_dig             <  to_date('").append(session.getAttribute("dt_final")).append("','dd/MM/yyyy')+1 ");

          }
          sbcatr.append("    AND res.lin_cdgo             =  nvl(").append(session.getAttribute("lin_cdgo")).append(",res.lin_cdgo) ");
          sbcatr.append("    AND res.codigo_marca         =  nvl(").append(session.getAttribute("codigo_marca")).append(",res.codigo_marca) ");
          sbcatr.append("    AND cli.cli_cdgo             =  res.cli_cdgo ");
          sbcatr.append("    AND rep.rep_cdgo             =  res.rep_cdgo ");
          sbcatr.append("    AND res.esc_seqn             =  clr.codigo_sequencial ");
          sbcatr.append("    AND rm.marca_produto         =  res.codigo_marca ");
          sbcatr.append("    AND clr.codigo_representante =  rep.rep_cdgo ");
          sbcatr.append("    AND cli.cli_cdgo             =  clr.codigo_cliente ");
          sbcatr.append("    AND rm.codigo_representante  =  rep.rep_cdgo ");
          sbcatr.append("    AND res.codigo_regional      =  nvl(").append(r.getCodigo_regional()).append(",res.codigo_regional) ");
          sbcatr.append("    AND cli.cli_situ             =  nvl('A',cli.cli_situ) ");
          sbcatr.append("    AND cli.seg_cdgo             NOT IN (9,153) ");
          sbcatr.append("    AND rep.rep_situ             =  nvl(").append(session.getAttribute("situacao_representante")).append(",rep.rep_situ) ");
          sbcatr.append("    AND clr.situacao             =  nvl(").append(session.getAttribute("situacao_cliente_representante")).append(",clr.situacao) ");
          sbcatr.append("    AND rm.situacao              =  nvl(").append(session.getAttribute("situacao_representante_marca")).append(",rm.situacao) ");
          sbcatr.append("    AND NOT EXISTS (SELECT 1 ");
          sbcatr.append("                      FROM ped ped ");
          sbcatr.append("                     WHERE ped.cli_cdgo = cli.cli_cdgo ");
          sbcatr.append("                       AND ped.rep_cdgo = rep.rep_cdgo ");
          sbcatr.append("                       AND ped.ped_especie_pedido <> 'C' ");
          if (session.getAttribute("ca_abrir_meses") == "S") {
              sbcatr.append("                       AND ped.ped_dadg >= to_date('"+r.getMes()+"', 'yyyy-mm-dd hh24:mi:ss') ");
              sbcatr.append("                       AND ped.ped_dadg  < last_day(to_date('"+r.getMes()+"', 'yyyy-mm-dd hh24:mi:ss')) + 1 ");
          }else{
              sbcatr.append("                       AND ped.ped_dadg >= TO_DATE('").append(session.getAttribute("dt_inicial")).append("', 'dd/MM/yyyy') ");
              sbcatr.append("                       AND ped.ped_dadg <= TO_DATE('").append(session.getAttribute("dt_final")).append("', 'dd/MM/yyyy') + 1 ");
          }
          sbcatr.append("                       AND NOT EXISTS (SELECT 1 ");
          sbcatr.append("                                     FROM ped pdd ");
          sbcatr.append("                                    WHERE pdd.ped_nmro = ped.ped_recolocado ");
          sbcatr.append("                                      AND pdd.ped_dadg < TO_DATE('").append(session.getAttribute("dt_inicial")).append("', 'dd/MM/yyyy'))) ");
          sbcatr.append("     ) ");
        ...
        */
        ...
        sbcat.append(" SELECT /*+ all_rows */ COUNT(distinct cli_cdgo) atendidos_qtd ");
        sbcat.append("   FROM (  ");
        sbcat.append(" SELECT cli.cli_cdgo ");
        sbcat.append("   FROM resumo_ped res ");
        sbcat.append("      , cli ");
        sbcat.append("      , rep ");
        sbcat.append("      , cliente_representante clr ");
        sbcat.append("      , representante_marca rm ");
        if (session.getAttribute("ca_abrir_meses") == "S") {
          sbcat.append("  WHERE res.data_dig >= to_date('"+r.getMes() + "', 'yyyy-mm-dd hh24:mi:ss') ");
          sbcat.append("  AND res.data_dig  < last_day(to_date('"+r.getMes() + "', 'yyyy-mm-dd hh24:mi:ss')) + 1 ");
        } else {
          sbcat.append("  WHERE res.data_dig             >= to_date('").append(session.getAttribute("dt_inicial")).append("','dd/MM/yyyy') ");
          sbcat.append("    AND res.data_dig             <  to_date('").append(session.getAttribute("dt_final")).append("','dd/MM/yyyy')+1 ");
        }
        sbcat.append("    AND res.lin_cdgo             =  nvl(").append(session.getAttribute("lin_cdgo")).append(",res.lin_cdgo) ");
        sbcat.append("    AND res.codigo_marca         =  nvl(").append(session.getAttribute("codigo_marca")).append(",res.codigo_marca) ");
        sbcat.append("    AND cli.cli_cdgo             =  res.cli_cdgo ");
        sbcat.append("    AND rep.rep_cdgo             =  res.rep_cdgo ");
        sbcat.append("    AND res.esc_seqn             =  clr.codigo_sequencial ");
        sbcat.append("    AND rm.marca_produto         =  res.codigo_marca ");
        sbcat.append("    AND clr.codigo_representante =  rep.rep_cdgo ");
        sbcat.append("    AND cli.cli_cdgo             =  clr.codigo_cliente ");
        sbcat.append("    AND rm.codigo_representante  =  rep.rep_cdgo ");
        sbcat.append("    AND res.codigo_regional      =  nvl(").append(r.getCodigo_regional()).append(",res.codigo_regional) ");
        sbcat.append("    AND cli.cli_situ             =  nvl('A',cli.cli_situ) ");
        sbcat.append("    AND cli.seg_cdgo             NOT IN (9,153) ");
        sbcat.append("    AND rep.rep_situ             =  nvl(").append(session.getAttribute("situacao_representante")).append(",rep.rep_situ) ");
        sbcat.append("    AND clr.situacao             =  nvl(").append(session.getAttribute("situacao_cliente_representante")).append(",clr.situacao) ");
        sbcat.append("    AND rm.situacao              =  nvl(").append(session.getAttribute("situacao_representante_marca")).append(",rm.situacao) ");
        sbcat.append("  UNION ALL ");
        sbcat.append(" SELECT cli.cli_cdgo ");
        sbcat.append("   FROM resumo_pedidos res ");
        sbcat.append("      , cli ");
        sbcat.append("      , rep ");
        sbcat.append("      , cliente_representante clr ");
        sbcat.append("      , representante_marca rm ");
        ...
        sbcat.append("    AND rm.situacao              =  nvl(").append(session.getAttribute("situacao_representante_marca")).append(",rm.situacao) ");
        /*
            sbcat.append("    AND NOT EXISTS (SELECT 1 ");
            sbcat.append("                      FROM ped ped ");
            sbcat.append("                     WHERE ped.cli_cdgo = cli.cli_cdgo ");
            sbcat.append("                       AND ped.rep_cdgo = rep.rep_cdgo ");
            sbcat.append("                       AND ped.ped_especie_pedido = 'C' ");
            if (session.getAttribute("ca_abrir_meses") == "S") {
                sbcat.append("  AND  ped.ped_dadg >= to_date('"+r.getMes()+"', 'yyyy-mm-dd hh24:mi:ss') ");
                sbcat.append("  AND  ped.ped_dadg  < last_day(to_date('"+r.getMes()+"', 'yyyy-mm-dd hh24:mi:ss')) + 1 ");
            }else{
                sbcat.append("                       AND ped.ped_dadg >= TO_DATE('").append(session.getAttribute("dt_inicial")).append("', 'dd/MM/yyyy') ");
                sbcat.append("                       AND ped.ped_dadg <= TO_DATE('").append(session.getAttribute("dt_final")).append("', 'dd/MM/yyyy') + 1 ");
            }
            sbcat.append("                       AND EXISTS (SELECT 1 ");
            sbcat.append("                                     FROM ped pdd ");
            sbcat.append("                                    WHERE pdd.ped_nmro = ped.ped_recolocado ");
            sbcat.append("                                      AND pdd.ped_dadg < TO_DATE('").append(session.getAttribute("dt_inicial")).append("', 'dd/MM/yyyy'))) ");
            */
        ...
        if (session.getAttribute("ca_consulta_clientes") == "S") {
          Vector res_Clientes_atendidos = j.select(table_clientes_atendidos, sbcat.toString(), null);
        ...
        //-----cidades ativas-------
        StringBuilder sbcidat = new StringBuilder();
        sbcidat.append(" SELECT /*+ all_rows */ count(distinct ecl.ecl_cdad||' - '||ecl.est_unifed) cidades_ativas_qtd ");
        sbcidat.append("   FROM ecl ");
        sbcidat.append("      , cliente_representante cr ");
        sbcidat.append("      , rep r ");
        sbcidat.append("      , representante_marca rm ");
        sbcidat.append("      , cli c ");
        sbcidat.append("  WHERE ecl.ned_cdgo            in ('COB', 'UNI') ");
        sbcidat.append("    AND ecl.cli_cdgo            =  cr.codigo_cliente ");
        sbcidat.append("    AND cr.codigo_representante =  r.rep_cdgo ");
        sbcidat.append("    AND r.rep_cdgo              =  rm.codigo_representante ");
        sbcidat.append("    AND c.cli_cdgo              =  cr.codigo_cliente ");
        sbcidat.append("    AND cr.codigo_sequencial    =  ecl.esc_seqn ");
        sbcidat.append("    AND c.cli_situ              =  nvl('A', c.cli_situ) ");
        sbcidat.append("    AND c.seg_cdgo              NOT IN (9,153) ");
        sbcidat.append("    AND r.rep_situ              =  nvl(").append(session.getAttribute("situacao_representante")).append(",r.rep_situ) ");
        sbcidat.append("    AND cr.situacao             =  nvl(").append(session.getAttribute("situacao_cliente_representante")).append(",cr.situacao) ");
        sbcidat.append("    AND rm.situacao             =  nvl(").append(session.getAttribute("situacao_representante_marca")).append(",rm.situacao) ");
        //  sbcidat.append("    AND r.codigo_regional       =  nvl(").append(r.getCodigo_regional()).append(",r.codigo_regional) ");
        sbcidat.append("    AND r.codigo_regional       =  ").append(r.getCodigo_regional());
        sbcidat.append("    AND rm.marca_produto        =  nvl(").append(session.getAttribute("codigo_marca")).append(",rm.marca_produto) ");

        if (session.getAttribute("ca_consulta_cidades") == "S") {
          Vector res_Cidades_ativas = j.select(table_cidades_ativas, sbcidat.toString(), null);
        ...
        StringBuilder sbcidaten = new StringBuilder();
        sbcidaten.append(" SELECT /*+ all_rows */ COUNT(distinct cidade) cidades_atendidas_qtd ");
        sbcidaten.append(" FROM ( ");
        sbcidaten.append(" SELECT DISTINCT(ecl.ecl_cdad || ' - ' || ecl.est_unifed) cidade ");
        sbcidaten.append("   FROM resumo_ped res ");
        sbcidaten.append("      , ecl ");
        sbcidaten.append("      , cli ");
        sbcidaten.append("      , rep ");
        sbcidaten.append("      , cliente_representante clr ");
        sbcidaten.append("      , representante_marca rm ");
        if (session.getAttribute("ca_abrir_meses") == "S") {
          sbcidaten.append("  WHERE res.data_dig >= to_date('"+r.getMes() + "', 'yyyy-mm-dd hh24:mi:ss') ");
          sbcidaten.append("  AND res.data_dig  < last_day(to_date('"+r.getMes() + "', 'yyyy-mm-dd hh24:mi:ss')) + 1 ");
        } else {
          sbcidaten.append("  WHERE res.data_dig             >= to_date('").append(session.getAttribute("dt_inicial")).append("','dd/MM/yyyy') ");
          sbcidaten.append("    AND res.data_dig             <  to_date('").append(session.getAttribute("dt_final")).append("','dd/MM/yyyy')+1 ");
        }
        sbcidaten.append("    AND res.lin_cdgo             =  nvl(").append(session.getAttribute("lin_cdgo")).append(",res.lin_cdgo) ");
        sbcidaten.append("    AND res.codigo_marca         =  nvl(").append(session.getAttribute("codigo_marca")).append(",res.codigo_marca) ");
        sbcidaten.append("    AND ecl.cli_cdgo             =  res.cli_cdgo ");
        sbcidaten.append("    AND cli.cli_cdgo             =  res.cli_cdgo ");
        sbcidaten.append("    AND rep.rep_cdgo             =  res.rep_cdgo ");
        sbcidaten.append("    AND clr.codigo_cliente       =  cli.cli_cdgo ");
        sbcidaten.append("    AND rm.codigo_representante  =  rep.rep_cdgo ");
        sbcidaten.append("    AND clr.codigo_representante =  rep.rep_cdgo ");
        sbcidaten.append("    AND clr.codigo_sequencial    =  res.esc_seqn ");
        sbcidaten.append("    AND rm.marca_produto         =  res.codigo_marca ");
        sbcidaten.append("    AND res.codigo_regional      =  nvl(").append(r.getCodigo_regional()).append(",res.codigo_regional) ");
        sbcidaten.append("    AND ecl.ned_cdgo             in ('COB', 'UNI') ");
        sbcidaten.append("    AND ecl.esc_seqn             =  res.esc_seqn ");
        sbcidaten.append("    AND cli.cli_situ             =  nvl('A',cli.cli_situ) ");
        sbcidaten.append("    AND cli.seg_cdgo             NOT IN (9,153) ");
        sbcidaten.append("    AND rep.rep_situ             =  nvl(").append(session.getAttribute("situacao_representante")).append(",rep.rep_situ) ");
        sbcidaten.append("    AND clr.situacao             =  nvl(").append(session.getAttribute("situacao_cliente_representante")).append(",clr.situacao) ");
        sbcidaten.append("    AND rm.situacao              =  nvl(").append(session.getAttribute("situacao_representante_marca")).append(",rm.situacao) ");
        sbcidaten.append("  UNION ALL ");
        sbcidaten.append(" SELECT DISTINCT(ecl.ecl_cdad || ' - ' || ecl.est_unifed) cidade ");
        sbcidaten.append("   FROM resumo_pedidos res ");
        sbcidaten.append("      , ecl ");
        sbcidaten.append("      , cli ");
        sbcidaten.append("      , rep ");
        sbcidaten.append("      , cliente_representante clr ");
        ...
        if (session.getAttribute("ca_consulta_cidades") == "S") {
          Vector res_Cidades_atendidas = j.select(table_cidades_atendidas, sbcidaten.toString(), null);
    ...
    session.setAttribute("res_Clientes_regional",res_Clientes_regional);
    if (!(res_Clientes_regional != null && res_Clientes_regional.size() > 0)) {
      ActionErrors errors = (ActionErrors) request.getAttribute(org.apache.struts.Globals.ERROR_KEY);
      if (errors == null) {
        errors = new ActionErrors();
      }
      errors.add("ActionErrors.GLOBAL_ERROR_bl_form_Clientes_regional", new ActionMessage("warn.norowsselected",""));
      request.setAttribute(org.apache.struts.Globals.ERROR_KEY, errors);
    }
