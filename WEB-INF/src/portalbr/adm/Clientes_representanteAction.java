...
public class Clientes_representanteAction extends com.egen.util.struts.AbstractAction {
  ...
  private void select_action(JdbcUtil j, ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
    ...
    Vector res_Clientes_representante = j.select(table, query.toString(), null);


    if (res_Clientes_representante != null && res_Clientes_representante.size() > 0) {
      for (int x = 0; x < res_Clientes_representante.size(); x++) {
        portalbr.dbobj.view.Clientes_representante r = (portalbr.dbobj.view.Clientes_representante) res_Clientes_representante.elementAt(x);
        ...
        sbga.append("    AND repr.rep_cdgo             = '").append(r.getRep_cdgo()).append("'");
        ...
        sbca.append("    AND rep.rep_cdgo = nvl('").append(r.getRep_cdgo()).append("',rep.rep_cdgo) ");
        ...
        /*
          ...
          sbcatr.append("    AND res.rep_cdgo              =  nvl('").append(r.getRep_cdgo()).append("',res.rep_cdgo ) ");
        ...
        */
        ...
        sbcat.append("    AND res.rep_cdgo              =  nvl('").append(r.getRep_cdgo()).append("',res.rep_cdgo ) ");
        ...
        */
        ...
        StringBuilder sbciat = new StringBuilder();
        sbciat.append(" SELECT /*+ all_rows */ COUNT(distinct ecl.ecl_cdad||' - '||ecl.est_unifed) cidades_ativas_qtd ");
        sbciat.append("   FROM ecl ");
        sbciat.append("      , cliente_representante cr ");
        sbciat.append("      , rep r ");
        sbciat.append("      , representante_marca rm ");
        sbciat.append("      , cli c ");
        sbciat.append("  WHERE ecl.ned_cdgo            IN ('COB', 'UNI') ");
        sbciat.append("    AND ecl.cli_cdgo            =  cr.codigo_cliente ");
        sbciat.append("    AND cr.codigo_representante =  r.rep_cdgo ");
        sbciat.append("    AND r.rep_cdgo              =  rm.codigo_representante ");
        sbciat.append("    AND c.cli_cdgo              =  cr.codigo_cliente ");
        sbciat.append("    AND cr.codigo_sequencial    =  ecl.esc_seqn ");
        sbciat.append("    AND c.cli_situ              =  nvl('A',c.cli_situ) ");
        sbciat.append("    AND c.seg_cdgo              NOT IN (9,153) ");
        sbciat.append("    AND r.rep_situ              =  nvl(").append(session.getAttribute("situacao_representante")).append(",r.rep_situ) ");
        sbciat.append("    AND cr.situacao             =  nvl(").append(session.getAttribute("situacao_cliente_representante")).append(",cr.situacao) ");
        sbciat.append("    AND rm.situacao             =  nvl(").append(session.getAttribute("situacao_representante_marca")).append(",rm.situacao) ");
        sbciat.append("    AND r.codigo_regional       =  nvl(").append(r.getCodigo_regional()).append(",r.codigo_regional) ");
        sbciat.append("    AND r.rep_cdgo              =  '").append(r.getRep_cdgo()).append("'");

        if (session.getAttribute("ca_consulta_cidades") == "S") {
          Vector res_Cidades_ativas = j.select(table_cidades_ativas, sbciat.toString(), null);
        ...
        //-----cidades atendidas-------
        StringBuilder sbciaten = new StringBuilder();
        sbciaten.append(" SELECT /*+ all_rows */ COUNT(distinct cidade) cidades_atendidas_qtd ");
        sbciaten.append(" FROM ( ");
        sbciaten.append(" SELECT DISTINCT(ecl.ecl_cdad || ' - ' || ecl.est_unifed) cidade ");
        sbciaten.append("   FROM resumo_ped res ");
        sbciaten.append("      , ecl ");
        sbciaten.append("      , cli ");
        sbciaten.append("      , rep ");
        sbciaten.append("      , cliente_representante clr ");
        sbciaten.append("      , representante_marca rm ");
        sbciaten.append("  WHERE res.data_dig             >= to_date('").append(session.getAttribute("dt_inicial")).append("','dd/MM/yyyy') ");
        sbciaten.append("    AND res.data_dig             <  to_date('").append(session.getAttribute("dt_final")).append("','dd/MM/yyyy')+1 ");
        sbciaten.append("    AND res.lin_cdgo             =  nvl(").append(session.getAttribute("lin_cdgo")).append(",res.lin_cdgo) ");
        sbciaten.append("    AND res.codigo_marca         =  nvl(").append(session.getAttribute("codigo_marca")).append(",res.codigo_marca) ");
        sbciaten.append("    AND ecl.cli_cdgo             =  res.cli_cdgo ");
        sbciaten.append("    AND cli.cli_cdgo             =  res.cli_cdgo ");
        sbciaten.append("    AND rep.rep_cdgo             =  res.rep_cdgo ");
        sbciaten.append("    AND clr.codigo_cliente       =  cli.cli_cdgo ");
        sbciaten.append("    AND rm.codigo_representante  =  rep.rep_cdgo ");
        sbciaten.append("    AND clr.codigo_representante =  rep.rep_cdgo ");
        sbciaten.append("    AND clr.codigo_sequencial    =  res.esc_seqn ");
        sbciaten.append("    AND rm.marca_produto         =  res.codigo_marca ");
        sbciaten.append("    AND res.rep_cdgo             =  nvl('").append(r.getRep_cdgo()).append("',res.rep_cdgo) ");
        sbciaten.append("    AND res.codigo_regional      =  nvl(").append(r.getCodigo_regional()).append(",res.codigo_regional) ");
        sbciaten.append("    AND ecl.ned_cdgo             in ('COB', 'UNI') ");
        sbciaten.append("    AND ecl.esc_seqn             =  res.esc_seqn ");
        sbciaten.append("    AND cli.cli_situ             =  nvl('A',cli.cli_situ) ");
        sbciaten.append("    AND cli.seg_cdgo             NOT IN (9,153) ");
        sbciaten.append("    AND rep.rep_situ             =  nvl(").append(session.getAttribute("situacao_representante")).append(",rep.rep_situ) ");
        sbciaten.append("    AND clr.situacao             =  nvl(").append(session.getAttribute("situacao_cliente_representante")).append(",clr.situacao) ");
        sbciaten.append("    AND rm.situacao              =  nvl(").append(session.getAttribute("situacao_representante_marca")).append(",rm.situacao) ");
        sbciaten.append("  UNION ALL ");
        sbciaten.append(" SELECT DISTINCT(ecl.ecl_cdad || ' - ' || ecl.est_unifed) cidade ");
        sbciaten.append("   FROM resumo_pedidos res ");
        sbciaten.append("      , ecl ");
        sbciaten.append("      , cli ");
        sbciaten.append("      , rep ");
        sbciaten.append("      , cliente_representante clr ");
        ...
        if (session.getAttribute("ca_consulta_cidades") == "S") {
          Vector res_Cidades_atendidas = j.select(table_cidades_atendidas, sbciaten.toString(), null);
