...
/**
 * Creation Date 23/12/2005 09:36:58
 * Last Modify Date 03/10/2016 15:34:48
 */

public class Clientes_marca_regionalAction extends com.egen.util.struts.AbstractAction {
  ...
  public ActionForward perform_select_action(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
    ...
    try {
      ...
      errors.add("ActionErrors.GLOBAL_ERROR_bl_form_Clientes_marca_regional", new ActionMessage("error.action.exception",com.egen.util.system.Error.getMessage(e)));
  ...
  private void select_action(JdbcUtil j, ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
    HttpSession session = request.getSession(true);
    Clientes_marca_regionalActionForm f = (Clientes_marca_regionalActionForm) form;
    portalbr.dbobj.view.Clientes_marca_regional table = new portalbr.dbobj.view.Clientes_marca_regional();
    ...
    Vector res_Clientes_marca_regional = j.select(table, query.toString(), null);


    if (res_Clientes_marca_regional != null && res_Clientes_marca_regional.size() > 0) {
      for (int x = 0; x < res_Clientes_marca_regional.size(); x++) {
        portalbr.dbobj.view.Clientes_marca_regional r = (portalbr.dbobj.view.Clientes_marca_regional) res_Clientes_marca_regional.elementAt(x);
        ...
        qgativos.append("    AND mapr.codigo_marca         = nvl(").append(session.getAttribute("codigo_marca")).append(",mapr.codigo_marca) ");
        qgativos.append("    AND repr.codigo_regional      = nvl(").append(session.getAttribute("codigo_regional")).append(",repr.codigo_regional) ");
        ...
        //-----clientes ativos-------
        StringBuilder qativos = new StringBuilder();
        qativos.append(" SELECT /*+ all_rows */ count(distinct cli.cli_cdgo) ativos_qtd ");
        qativos.append("   FROM rep ");
        qativos.append("      , cliente_representante clr ");
        qativos.append("      , cli ");
        qativos.append("      , representante_marca rm ");
        qativos.append("  WHERE clr.codigo_representante =  rep.rep_cdgo ");
        qativos.append("    AND cli.cli_cdgo             =  clr.codigo_cliente ");
        qativos.append("    AND rm.codigo_representante  =  rep.rep_cdgo ");
        qativos.append("    AND cli.cli_situ             =  nvl('A', cli.cli_situ) ");
        qativos.append("    AND cli.seg_cdgo             NOT IN (9,153) ");
        qativos.append("    AND clr.situacao             =  nvl(").append(session.getAttribute("situacao_cliente_representante")).append(",clr.situacao) ");
        qativos.append("    AND rm.situacao              =  nvl(").append(session.getAttribute("situacao_representante_marca")).append(",rm.situacao) ");
        qativos.append("    AND rep.rep_situ             =  nvl(").append(session.getAttribute("situacao_representante")).append(",rep.rep_situ) ");
        qativos.append("    AND rm.marca_produto         =  nvl(").append(r.getCodigo_marca()).append(", rm.marca_produto) ");
        qativos.append("    AND rep.codigo_regional      =  nvl(").append(r.getCodigo_regional()).append(", rep.codigo_regional) ");
        qativos.append("    AND rm.marca_produto         =  nvl(").append(session.getAttribute("codigo_marca")).append(",rm.marca_produto) ");
        qativos.append("    AND rep.codigo_regional      =  nvl(").append(session.getAttribute("codigo_regional")).append(",rep.codigo_regional) ");

        if (session.getAttribute("ca_consulta_clientes") == "S") {
          Vector res_Clientes_ativos = j.select(table_clientes_ativos, qativos.toString(), null);
        ...
        //----- Clientes Atendidos com Recolocação -------
        /*
          ...
          qcatendidosr.append("    AND rep.codigo_regional      =  nvl(").append(session.getAttribute("codigo_regional")).append(",rep.codigo_regional) ");
        ...
        qcatendidos.append("    AND rep.codigo_regional      =  nvl(").append(session.getAttribute("codigo_regional")).append(",rep.codigo_regional) ");
        ...
        */
        ...
        qcativas.append("    AND rm.marca_produto        =  nvl(").append(session.getAttribute("codigo_marca")).append(",rm.marca_produto) ");
        qcativas.append("    AND r.codigo_regional       =  nvl(").append(session.getAttribute("codigo_regional")).append(",r.codigo_regional) ");
        ...
        qcatendidas.append("    AND rep.codigo_regional      =  nvl(").append(session.getAttribute("codigo_regional")).append(",rep.codigo_regional) ");
    ...
    session.setAttribute("res_Clientes_marca_regional",res_Clientes_marca_regional);
    if (!(res_Clientes_marca_regional != null && res_Clientes_marca_regional.size() > 0)) {
      ActionErrors errors = (ActionErrors) request.getAttribute(org.apache.struts.Globals.ERROR_KEY);
      if (errors == null) {
        errors = new ActionErrors();
      }
      errors.add("ActionErrors.GLOBAL_ERROR_bl_form_Clientes_marca_regional", new ActionMessage("warn.norowsselected",""));
      request.setAttribute(org.apache.struts.Globals.ERROR_KEY, errors);
    }
